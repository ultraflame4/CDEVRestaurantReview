...
class RestauRantDatabase {
    ...
    FindUser(email) {
        return new Promise((resolve, reject) => {
            this.#conn.query(`SELECT * FROM cdevrestaurantdatabase.users WHERE email=? LIMIT 1`,
                [email], (err, results) => {
                if (err) {
    ...
    GetReview(reviewId) {
        ...
        return new Promise((resolve, reject) => {
            this.#conn.query(`SELECT * FROM cdevrestaurantdatabase.reviews WHERE id=? LIMIT 1`,
